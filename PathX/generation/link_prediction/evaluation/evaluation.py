...
from link_prediction.models.transe_refactor import TransE
from link_prediction.models.model import Model, pathXModel
import numpy as np
...
class Evaluator:
    ...
    def evaluate(self,
        ...
        for i in range(samples.shape[0]):
            ...
            if ranks[i][1] == 1 :
                top_1_tp.add(f'{self.dataset.test_triples[i][0]}\t{self.dataset.test_triples[i][1]}\t{self.dataset.test_triples[i][2]}')
            if ranks[i][1] <= 10 :
                top_10_tp.add(f'{self.dataset.test_triples[i][0]}\t{self.dataset.test_triples[i][1]}\t{self.dataset.test_triples[i][2]}')
        ...
        return self.mrr(all_ranks), self.hits_at(all_ranks, 1), self.hits_at(all_ranks, 10), self.mr(all_ranks), top_1_tp, top_10_tp
...
class pathXEvaluator(Evaluator):

    def __init__(self, model: pathXModel):
