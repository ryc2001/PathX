...
from link_prediction.models.model import Model, OPTIMIZER_NAME, BATCH_SIZE, EPOCHS, LEARNING_RATE, DECAY_1, REGULARIZER_NAME, \
    REGULARIZER_WEIGHT, DECAY_2, pathXModel
...
class MultiClassNLLOptimizer(Optimizer):
    ...
    def train(self,
        ...
        # batch size must be the minimum between the passed value and the number of pathX training facts
    ...
    def step_on_batch(self, loss, batch):
        ...
        # reg_loss = reg_lambda * (torch.norm(self.model.entity_embeddings[self.model.pathX_entity_id2,:] - self.model.entity_embeddings[self.model.original_entity_id2,:], p=2) ** 2)
        #
        # # 总损失
        # l += reg_loss
...
class pathXMultiClassNLLOptimizer(MultiClassNLLOptimizer):
    def __init__(self,
                 model: pathXModel,
                 hyperparameters: dict,
                 verbose: bool = True):
