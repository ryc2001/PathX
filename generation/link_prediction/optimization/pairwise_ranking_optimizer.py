...
class PairwiseRankingOptimizer(Optimizer):
    ...
    def save_model(self, save_path: str):
        """
        保存模型的状态，包括模型参数、优化器参数和超参数
        """
        state = {
            'state_dict': self.model.state_dict(),  # 模型参数
            'optimizer': self.optimizer.state_dict(),  # 优化器参数
            'hyperparameters': self.hyperparameters  # 超参数字典
        }
        torch.save(state, save_path)  # 保存到指定路径
        if hasattr(self, 'logger'):  # 如果定义了日志记录器
            self.logger.info('Saving model to {0}'.format(save_path))
        else:
    ...
    def train(self,
        ...
        for e in range(1, self.epochs+1):
            ...
            if evaluate_every > 0 and valid_samples is not None and e % evaluate_every == 0:
                self.model.eval()
                with torch.no_grad():
                    mrr, h1, h10, mr,top_1,top_10 = self.evaluator.evaluate(samples=valid_samples, write_output=False)
                ...
                #     print("\t saving model_way1...")
                #     self.save_model(save_path)  # 调用保存模型的方法
                #     print("\t done.")
                if save_path is not None:
                   print("\t saving model...")
...
class pathXPairwiseRankingOptimizer(PairwiseRankingOptimizer):
    def __init__(self,
                 model:pathXModel,
                 hyperparameters: dict,
                 verbose: bool = True):

        super(pathXPairwiseRankingOptimizer, self).__init__(model=model,
                                                             hyperparameters=hyperparameters,
                                                             verbose=verbose)
        self.pathX_entity_id = model.pathX_entity_id
    ...
    def epoch(self,
        ...
        # pathX_entity_is_head = torch.where(all_positive_samples[:, 0] == self.pathX_entity_id, torch.ones(len(all_positive_samples)), torch.zeros(len(all_positive_samples)))
        # corrupted_heads = torch.where(pathX_entity_is_head == 0, random_entities, all_positive_samples[:, 0].type(torch.LongTensor))
        # corrupted_tails = torch.where(pathX_entity_is_head == 1, random_entities, all_positive_samples[:, 2].type(torch.LongTensor))
        ...
        with tqdm.tqdm(total=len(training_samples), unit='ex', disable=not self.verbose) as bar:
            ...
            while batch_start < len(training_samples):
                ...
                # THIS IS EXTREMELY IMPORTANT BECAUSE THIS WILL PROPAGATE THE UPDATES IN THE pathX ENTITY EMBEDDING
