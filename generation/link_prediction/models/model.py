...
class Model(nn.Module):
    ...
    def pathx_model_class(self):
        """
            This method provides the pathXModel implementation class corresponding to this specific Model class.
            E.g. ComplEx.pathX_model_class() -> pathXComplEx.__class__

            When called on a pathXModel subclass, it raises an exception.

            :return: The pathXModel class that corresponds to the Model Class running this method
            :raise: an Exception if called on a pathXModel subclass
        """
        pass


class pathXModel(Model):
    """
        The pathXModel class provides the interface that any post-trainable LP model should implement.

        The main functions of pathXModel are thus identical to Model
        (which is why pathXModel extends Model).

        In addition to that, a pathXModels also
    """
    ...
    def predict_samples(self,
        ...
        :param original_mode: a boolean flag specifying whether to work in original_mode or to use the pathX entity
    ...
    def update_embeddings(self):
        with torch.no_grad():
            self.entity_embeddings[self.pathX_entity_id] = self.pathX_entity_embedding
    ...
    def train(self, mode=True):
        ...
        In pathXModels, in post-training any layers, including BatchNorm1d or BatchNorm2d,
    ...
    def pathx_model_class(self):
        raise Exception(self.__class__.name  + " is a pathXModel.")