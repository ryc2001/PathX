...
from pathX_dataset import pathXDataset
from link_prediction.models.model import Model, DIMENSION, pathXModel
...
class TransE(Model):
    ...
    def __init__(self,
        ...
        # note: the init_random parameter is important because when initializing a pathXComplEx,
        ...
        # We do not use the torch.Embeddings module here in order to keep the code uniform to the pathXDistMult model,
    ...
    def forward(self, samples: np.array):
        ...
        """
        # print("relation_embeddings size:", self.entity_embeddings.size())
        # print("samples[:, 0]:", samples[:, 0])
        # print("max index in samples[:, 0]:", torch.max(samples[:, 0]))
        # print("min index in samples[:, 0]:", torch.min(samples[:, 0]))
    ...
    def pathX_model_class(self):
        return pathXTransE

################

class pathXTransE(pathXModel, TransE):
    def __init__(
            self,
        ...
        self.pathX_entity_id = dataset.pathX_entity_id
        self.original_entity_id2 = dataset.original_entity_id2
        self.pathX_entity_id2 = dataset.pathX_entity_id2
        ...
        # the tensor from which to initialize the pathX_entity_embedding;
        # if it is None it is initialized randomly
        if init_tensor1 is None:
            init_tensor1 = torch.rand(1, self.dimension)
            init_tensor2 = torch.rand(1, self.dimension)

        # It is *extremely* important that pathX_entity_embedding is both a Parameter and an instance variable
        ...
        #       self.pathX_entity_embedding = Parameter(torch.rand(1, self.dimension), requires_grad=True).cuda()
        # IT WOULD NOT WORK because cuda() returns a Tensor, not a Parameter.
        # Therefore pathX_entity_embedding would not be a Parameter anymore.
        self.pathX_entity_embedding = Parameter(init_tensor1.cuda(), requires_grad=True)
        self.pathX_entity_embedding2 = Parameter(init_tensor2.cuda(), requires_grad=True)
        with torch.no_grad():           # Initialize as any other embedding
            xavier_normal_(self.pathX_entity_embedding)
            xavier_normal_(self.pathX_entity_embedding2)

        self.relation_embeddings = frozen_relation_embeddings
        # self.entity_embeddings = self._create_trainable_entity_embeddings(frozen_entity_embeddings, dataset.pathX_train_ent)
        # self.entity_embeddings = model.entity_embeddings
        # self.entity_embeddings = torch.cat([self.entity_embeddings, self.pathX_entity_embedding,self.pathX_entity_embedding2], 0)
        # self.entity_embeddings = torch.nn.Parameter(
        #     torch.cat([self.entity_embeddings, self.pathX_entity_embedding, self.pathX_entity_embedding2], 0),
        #     requires_grad=True
        # )
        # self.entity_embeddings = torch.nn.Parameter(
        #     torch.cat([model.entity_embeddings.clone().detach(), init_tensor1.cuda(), init_tensor2.cuda()], 0),
        #     requires_grad=True
        # )
        # self.entity_embeddings = torch.cat([frozen_entity_embeddings, self.pathX_entity_embedding], 0)
        self.entity_embeddings = torch.nn.Parameter(
            torch.cat(
                [model.entity_embeddings.clone().detach(), self.pathX_entity_embedding,self.pathX_entity_embedding2],
                0),
            requires_grad=True
        )
    def _create_trainable_entity_embeddings(self, frozen_embeddings, trainable_indices):
        """
        根据指定的下标列表创建新的实体嵌入矩阵，其中指定下标的实体会被随机重新初始化并可训练。

        :param frozen_embeddings: 冻结的实体嵌入（不可更新）
        :param trainable_indices: 需要更新的实体下标列表
        :return: 新的实体嵌入矩阵
        """
        # 克隆冻结的实体嵌入
        entity_embeddings = frozen_embeddings.clone()

        # 将指定下标的实体嵌入重新随机初始化，并替换为可训练参数
        for idx in trainable_indices:
            # 随机初始化
            random_embedding = torch.rand(1, self.dimension).cuda()
            xavier_normal_(random_embedding)

            # 替换为可训练参数
            # entity_embeddings[idx] = Parameter(random_embedding.squeeze(0), requires_grad=True)
            # entity_embeddings[idx] = Parameter(frozen_embeddings[idx].clone(), requires_grad=True)
            entity_embeddings[idx] = frozen_embeddings[idx].clone().detach().requires_grad_(True)
    ...
    def predict_samples(self,
        ...
        either in original_mode (i.e., ignoring the pathX entity, or mimic, and using the original embedding instead)
        ...
        # if we are in original_mode, make sure that the pathX entity is not featured in the samples to predict
        # otherwise, make sure that the original entity is not featured in the samples to predict
        forbidden_entity_id = self.pathX_entity_id if original_mode else self.original_entity_id
        ...
        for i in range(len(direct_samples)):
            ...
            # remove the pathX entity id from the tail predictions  (note: it could be absent due to filtering)
