...
from link_prediction.models.model import Model, pathXModel, DIMENSION, INIT_SCALE
...
class ComplEx(Model):
    ...
    def __init__(self,
        ...
        # We do not use the torch.Embeddings module here in order to keep the code uniform to the pathXComplEx model,
        # (on which torch.Embeddings can not be used as they do not allow the LPFT-training mechanism).
...
class pathXComplEx(pathXModel, ComplEx):
    def __init__(
            self,
        ...
        # self.pathX_entity_embedding = Parameter( frozen_entity_embeddings[self.original_entity_id,:].unsqueeze(0), requires_grad=True)
        # self.pathX_entity_embedding2 = Parameter(frozen_entity_embeddings[self.original_entity_id2,:].unsqueeze(0), requires_grad=True)
        with torch.no_grad():  # Initialize as any other embedding
            self.pathX_entity_embedding *= self.init_scale
            self.pathX_entity_embedding2 *= self.init_scale

        self.relation_embeddings = frozen_relation_embeddings
        self.entity_embeddings = torch.nn.Parameter(
            torch.cat(
                [model.entity_embeddings.clone().detach(), self.pathX_entity_embedding],
                0),
            requires_grad=True
        )
    ...
    def _create_trainable_entity_embeddings(self, frozen_embeddings, trainable_indices):
        ...
        for idx in trainable_indices:
            ...
            random_embedding *= self.init_scale
    ...
    def predict_samples(self,
        ...
        which means ignoring in any circumstances the additional "fake" pathX entity.
